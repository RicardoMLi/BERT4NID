...
class LSTM(nn.Module):
    def __init__(self, input_dim, num_classes):
        ...
        self.lstm = nn.LSTM(input_dim, 64, batch_first=True, num_layers=2, dropout=0.5)
...
def train(epoch, train_loader, device, num_classes, input_dims):
    model = LSTM(input_dim=input_dims, num_classes=num_classes).to(device)
...
base_path = r'F:\入侵检测数据集\med_processed'
num_classes = 4
epoch = 20
...
for seed in [1, 2, 3]:
    ...
    model = LSTM(input_dim=input_dims, num_classes=num_classes)
