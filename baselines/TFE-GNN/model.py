...
class MixTemporalGNN(nn.Module):
    def __init__(self, num_classes, K, embedding_size=64, h_feats=128, dropout=0.2, downstream_dropout=0.0):
        ...
        self.gcn_out_dim = 4 * h_feats
        self.gated_filter = Cross_Gated_Info_Filter(in_size=self.gcn_out_dim, K=self.K)
    ...
    def forward(self, header_graph_data, payload_graph_data, labels):
        header_gcn_out = self.header_graphConv(header_graph_data, header_graph_data.ndata['feat']).reshape(
            labels.shape[0], config.FLOW_PAD_TRUNC_LENGTH, -1)[:, :self.K, :]
