...
def train(epoch, train_loader, device, num_classes, input_dims):
    model = DNN(input_dim=input_dims, num_classes=num_classes).to(device)
...
base_path = r'F:\datasets\mqtt_processed'
num_classes = 5
epoch = 40
...
for seed in [1, 2, 3]:
    ...
    model = DNN(input_dim=input_dims, num_classes=num_classes)
