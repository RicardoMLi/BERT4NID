...
from utils import should_omit_packet, read_pcap
...
def packet_to_sparse_array(packet, max_length=784):
...
def transform_pcap(path, output_path: Path = None, output_batch_size=10000):
    ...
    for i, packet in enumerate(read_pcap(path)):
        ...
        if arr is not None:
            ...
            label = Kitsune_Mapper[path.name.split(".")[0]]
            row = {
                "label": label,
